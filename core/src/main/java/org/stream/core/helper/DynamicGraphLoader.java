 ...
 */
@Data
public class DynamicGraphLoader {
    ...
    private DynamicGraphContext graphContext;
    ...
    public Graph loadGraphFromFile(final String path) throws GraphLoadException {
        InputStream inputStream = getClass().getResourceAsStream(DEFAULT_GRAPH_FILE_PATH_PREFIX + path);

        return loadGraph(inputStream);
    }

    public Graph loadGraphFromHttpServer(final String uri) throws GraphLoadException {
        try {
            URL url = new URL(uri);
            ...
            return loadGraph(httpURLConnection.getInputStream());
    ...
    private GraphConfiguration parseGraphConfiguration(final String compressedInputString, final Graph graph) throws GraphLoadException {
        ...
        graph.setOriginalDefinition(compressedInputString);
